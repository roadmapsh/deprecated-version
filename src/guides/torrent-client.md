...
![diagram showing the difference between client/server (all clients connecting to one server) and peer-to-peer (peers connecting to each other) relationships](/assets/guides/torrent-client/client-server-p2p.png)
...
![illustration of a desktop computer and laptop sitting at a pub](/assets/guides/torrent-client/trackers.png)
...
!["illustration of a file being cut with scissors into multiple pieces, starting with piece 0](/assets/guides/torrent-client/pieces.png)
...
![a name tag saying 'Hello my name is 86d4c80024a469be4c50bc5a102cf71780310074'](/assets/guides/torrent-client/info-hash.png)
...
![a file with a name tag saying 'info_hash' and a person with a name tag 'peer_id'](/assets/guides/torrent-client/info-hash-peer-id.png)
...
![diagram showing how 192, 0, 2, 123, 0x1A, 0xE1 can be interpreted as 192.0.1.123:6881](/assets/guides/torrent-client/address.png)
...
![Two computers communicating. One asks 'do you speak BitTorrent and have this file?' and the other replies 'I speak BitTorrent and have that file'](/assets/guides/torrent-client/handshake.png)
...
!["A cartoon in which person 1 says 'hello I would like piece number—' and person 2 grabs him by the neck and says '00 00 00 01 00 (choke)'](/assets/guides/torrent-client/choke.png)

#### Interpreting messages
A message has a length, an **ID** and a **payload**. On the wire, it looks like:

![A message with 4 byte for the length, 1 byte for ID, and an optional payload](/assets/guides/torrent-client/message.png)
...
![a coffee shop loyalty card with eight slots, with stamps on the first four slots and a stamp on the second to last slot, represented as 11110010](/assets/guides/torrent-client/bitfield.png)
...
![a flow chart of the download strategy](/assets/guides/torrent-client/download.png)
...
![Two email threads simulating peer connections. The thread on the left shows a request followed by a reply, repeated three times. The thread on the left sends three requests, and receives three replies in quick succession.](/assets/guides/torrent-client/pipelining.png)
